/*
	...
	ASSIGNMENT 1.
	CSC_322.
	A PARENT SPAWNS 10 CHILDREN AND THE PARENT CAN COMMINUCATE TO ANY CHILD AND A CHILD CAN COMMUNNICATE WITH THE PARENT USING SYSTEM SYSTEM CALLS kill() and signal().
...
void notice(int sig_number){
	char *name = strsignal(WTERMSIG(sig_number));
	printf("I caught a %s signal\n", name);
	exit(0);
}
...
int main(void)
{
	...
	for (int i = 0; i < n; ++i){
		...
		if(p < 0){
			...
			// children processes.
			if(signal(SIGINT, notice) == SIG_ERR){
				//SHOW ERROR
				perror("signal");
				exit(1);
			}
        	...
        	char send_data[100];
        	sprintf(send_data, "My process is %d and the square of the loop counter is %d", getpid(), i*i);

        	// child process send data to parent.
        	write(fd_c2p[1], send_data, 4 * sizeof(send_data));
        	close(fd_c2p[0]); // close read side.
			...
			// kill child processes of the given ID using signals
			kill(p, SIGINT);
	...
	// 		// change signal number to signal name
	// 		char *name = strsignal(WTERMSIG(status));
	// 		printf("%d was ended with a %s signal\n", processes[i], name);
	// 	}
	// 	else { 
	// 		printf("%d was not ended with a signal\n", processes[i]);
	// 	}
	// }
